...
const { ensureHomeroomOwnsParentParam } = require('../middleware/ownership.middleware');
const { auditAuthorizedAccess } = require('../middleware/auditAccess.middleware');

// All routes require TEACHER authentication
router.use(authenticateToken);
router.use(requireAnyPermission(['attendance:write:assigned', 'grades:write:assigned', 'school:manage']));
 ...
 */
router.get('/my-homeroom-classes', auditAuthorizedAccess('homeroom.read.my_classes'), homeroomController.getMyHomeroomClasses);
...
router.post(
  ...
  verifyHomeroomTeacher,
  auditAuthorizedAccess('homeroom.attendance.write'),
...
router.get(
  '/classes/:classId/attendance',
  verifyHomeroomTeacher,
  auditAuthorizedAccess('homeroom.attendance.read'),
...
router.get(
  '/classes/:classId/students',
  verifyHomeroomTeacher,
  auditAuthorizedAccess('homeroom.students.read.list'),
...
router.get(
  '/students/:studentId',
  verifyStudentInHomeroomClass,
  auditAuthorizedAccess('homeroom.students.read.single'),
...
router.post(
  ...
  verifyHomeroomTeacher,
  auditAuthorizedAccess('homeroom.parents.write.create'),
...
router.get(
  '/classes/:classId/parents',
  verifyHomeroomTeacher,
  auditAuthorizedAccess('homeroom.parents.read.list'),
...
router.get(
  ...
  auditAuthorizedAccess('homeroom.parents.read.single'),
...
router.put(
  '/parents/:parentId',
  ensureHomeroomOwnsParentParam,
  auditAuthorizedAccess('homeroom.parents.write.update'),
...
router.post(
  ...
  verifyHomeroomTeacher,
  auditAuthorizedAccess('homeroom.parents.write.add_child'),
...
router.delete(
  ...
  ensureHomeroomOwnsParentParam,
  verifyStudentInHomeroomClass,
  auditAuthorizedAccess('homeroom.parents.write.remove_child'),
