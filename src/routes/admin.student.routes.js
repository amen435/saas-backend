 ...
 * @access  SCHOOL_ADMIN
 */
router.post('/', requireAnyPermission(['users:write']), auditAuthorizedAccess('admin_students.write.create'), async (req, res) => {
  try {
 ...
 * @access  SCHOOL_ADMIN
 */
router.get('/', requireAnyPermission(['users:read']), auditAuthorizedAccess('admin_students.read.list'), async (req, res) => {
  try {
    const { schoolId } = req.user;
